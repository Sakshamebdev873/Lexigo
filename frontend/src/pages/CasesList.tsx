...
export default function CasesList() {
  ...
  const load = useCallback(() => {
    ...
    setErr(null);
    api
      .listCases({ search, stage, fromDate, toDate })
      .then(setCases)
  ...
  const onDelete = async (c: Case) => {
    ...
    if (!confirm(`Delete case "${c.caseTitle}"? This also removes its tasks.`))
      return;
    try {
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 py-16 text-center">
          ...
          <p className="text-slate-500 text-sm">
            Adjust your filters or start fresh with a new entry.
          </p>
        </div>
      ) : (
                ...
                >
                  ...
                  >
                    <div className="flex justify-end gap-2">
                      <Link
                        to={`/cases/${c._id}`}
                        className="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors"
                      >
                        View
                      </Link>
                      <Link
                        to={`/cases/${c._id}/edit`}
                        className="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-indigo-600 hover:bg-indigo-50 transition-colors"
                      >
                        Edit
                      </Link>
                      <button
